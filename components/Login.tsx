...
const Login: React.FC<LoginProps> = ({ onSignIn, authError }) => {
  ...
  const handleSignInClick = async () => {
    ...
    try {
      await onSignIn();
      // On successful sign-in, the component will unmount, so no need to reset state here.
    } catch (error) {
      // On failure, reset the signing-in state to allow the user to try again.
    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen p-4">
      <div className="text-center p-8 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-2xl shadow-lg max-w-md w-full">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white/10 text-slate-100 font-semibold py-3 px-4 rounded-lg transition-all duration-200 hover:bg-white/20 border border-white/20 shadow-md disabled:bg-slate-500/20 disabled:border-slate-500/30 disabled:cursor-wait"
        >
          {isSigningIn ? (
            <>
